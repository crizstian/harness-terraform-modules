      ...
      for env, values in variables.OVERRIDES.ENV : "${svc}_${env}" => {
        vars = merge(
          values,
          {
            ...
            org_id     = try(local.environments_service_overrides_org_id[env], "") != "" ? local.environment_org_id[env] : try(variables.SERVICE_DEFINITION.org_id, var.common_values.org_id)
            project_id = try(local.environments_service_overrides_prj_id[env], "") != "" ? local.environment_prj_id[env] : try(variables.SERVICE_DEFINITION.project_id, var.common_values.project_id)
