output "pipelines" {
  value = { for key, details in harness_platform_pipeline.pipeline : key => merge(
    ...
    length(keys(harness_platform_triggers.trigger)) > 0 ? { triggers = { for key, trigger in harness_platform_triggers.trigger : key => { identifier = trigger.identifier } if trigger.target_id == details.identifier } } : {}
    )
  }
}
