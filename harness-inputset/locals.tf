    ...
    for name, details in var.harness_platform_inputsets : [
      for svc, variables in var.harness_platform_services : [
        ...
        for pipe, values in try(variables.vars.PIPELINE, {}) : [
          ...
          } if contains(keys(variables.vars.artifacts), try(env_details.primary_artifact, "NOT_DEFINED")) && try(local.inpt_by_svc["${svc}_${name}"].environment_type, env_details.type) == env_details.type
