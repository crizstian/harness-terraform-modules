        ...
        for infra, details in try(var.connectors["${type}_connectors"], {}) : "${type}_${infra}" => {
          vars = merge(
            ...
            try(local.infrastructure_tpl_dp_id[type], {}),
            {
              ...
              env_id             = harness_platform_environment.environment[element([for k, v in toset(try(details.tags, [])) : replace(v, "environment:", "") if startswith(v, "environment:")], 0)].identifier
