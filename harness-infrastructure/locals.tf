  ...
  infrastructures = merge(
    [
      for type, values in var.harness_platform_infrastructures : {
        for infra, details in try(var.connectors["${type}_connectors"], {}) : "${type}_${infra}" => {
          vars = merge(
            ...
            try(local.infrastructure_tpl_dp_id[type], {}),
            {
              ...
              env_id             = try(harness_platform_environment.environment[element([for k, v in toset(try(details.tags, [])) : replace(v, "environment:", "") if startswith(v, "environment:")], 0)].identifier, "")
        ...
        } if can(harness_platform_environment.environment[element([for k, v in toset(try(details.tags, [])) : replace(v, "environment:", "") if startswith(v, "environment:")], 0)].identifier)
      }
    ]...
  )
