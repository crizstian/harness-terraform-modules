...
locals {
  docker_delegates = { for name, delegate in try(local.enabled_delegates.docker, {}) : name => {
    ...
    url_args          = can(delegate.org_id) ? can(delegate.proj_id) ? "${local.account_args}&orgIdentifier=${try(delegate.org_id, local.harness_organization_id)}&projectIdentifier=${delegate.proj_id}" : "${local.account_args}&orgIdentifier=${try(delegate.org_id, local.harness_organization_id)}" : "${local.account_args}"
    tokenName         = can(delegate.tokenName) ? delegate.tokenName : can(delegate.org_id) ? "default_token_${try(delegate.org_id, local.harness_organization_id)}" : "default_token"
...
locals {
  k8s_connectors = merge([for type, delegates in var.harness_platform_delegates : {
    for key, value in delegates : key => merge(
      value,
      {
        org_id     = try(delegate.org_id, local.harness_organization_id)
